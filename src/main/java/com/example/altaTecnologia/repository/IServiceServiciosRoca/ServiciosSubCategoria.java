...
public interface ServiciosSubCategoria extends JpaRepository<subcategoriaroca, Integer> {
	...
	@Query("SELECT new com.example.altaTecnologia.models.reporte(s.nombre,count(v.subCat)) FROM subcategoriaroca s LEFT JOIN vitacora v ON v.subCat=s.id WHERE v.Fecha BETWEEN ?1 and ?2 GROUP BY s.nombre ORDER BY v.Fecha")
